...
pub fn camo_url(config: &Configuration, url: &Url) -> Result<Url> {
    let camo_key = if let Some(key) = config.camo_key.clone() { key } else { return Ok(url.clone()) };
    let camo_host = if let Some(host) = config.camo_host.clone() { host } else { return Ok(url.clone()) };
