use crate::{Configuration, scraper::{ScrapeImage, ScrapeResult, from_url}};
use anyhow::Result;
use anyhow::Context;
use log::trace;
...
async fn try_intermediary_hires(config: &Configuration, mut images: Vec<ScrapeImage>) -> Result<Vec<ScrapeImage>> {
    for image in images.clone() {
        let (domain, object_uuid, object_name) = {
            ...
            (domain.to_string(), object_uuid.to_string(), object_name.to_string())
        };
        let built_url = format!("{domain}/intermediary/{object_uuid}/{object_name}", domain = domain, object_uuid = object_uuid, object_name = object_name);
...
async fn try_old_hires(config: &Configuration, source_url: Url, mut images: Vec<ScrapeImage>, camo: &Url) -> Result<Vec<ScrapeImage>> {
    let serial = &SERIAL_REGEX.captures(source_url.as_str());
    let serial = match serial {
        None => anyhow::bail!("no serial captured"),
        Some(serial) => &serial[1],
    };
    let base36 = radix_fmt::radix(serial.parse::<i64>()?, 36).to_string().to_lowercase();
    
    let built_url = format!("http://orig01.deviantart.net/x_by_x-d{base36}.png", base36 = base36);

    let client = crate::scraper::client_with_redir_limit(config, reqwest::redirect::Policy::none())?;
    let resp = client.get(built_url).send().await.context("old hires request failed")?;
    if let Some((_, loc)) = resp.headers().iter().find(|(name, _value)| name.as_str().to_lowercase() == "location") {
        let loc = loc.to_str()?;
        images.push(ScrapeImage{
