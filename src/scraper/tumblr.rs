...
use crate::{Configuration, camo::camo_url, scraper::{ScrapeImage, ScrapeResult, ScrapeResultData, from_url}};
...
pub async fn tumblr_scrape(config: &Configuration, url: &Url, db: &sled::Db) -> Result<Option<ScrapeResult>> {
...
async fn process_post(post_type: PostType, post: Value, db: &sled::Db, config: &Configuration, client: &Client) -> Result<Option<Vec<ScrapeImage>>> {
    match post_type {
...
async fn process_post_photo(post: Value, db: &sled::Db, config: &Configuration, client: &Client) -> Result<Option<Vec<ScrapeImage>>> {
            ...
            match photos {
                ...
                Some(photos) => {
                    ...
                    for photo in photos.iter() {
                        ...
                        let image = upsize(photo["original_size"]["url"].clone(), db, config, client).await?;
...
async fn upsize(image_url: Value, db: &sled::Db, config: &Configuration, client: &Client) -> Result<Option<Url>> {
